...
const AllTasks = () => {
  ...
  const currentFilterValues = useMemo(() => {
    ...
    return{
      ...
      taskType: logicalFilters.find((item) => item.field === 'taskType')?.value || '',
    }
  }, [filters])
      ...
      <Box mt="20px" sx={{display: 'flex', flexWrap: 'wrap', gap: 3}}>
        ...
        <Box mb={2} mt={3} display="flex" width="84%" justifyContent="space-between" flexWrap="wrap">
          <Box display="flex" gap={2} flexWrap="wrap" mb={{ xs: '20px', sm: 0 }}>
            <CustomButton
              title={`Sort task ${currentPrice === 'asc' ? '↑' : '↓'}`}
            ...
            <Select
              ...
              defaultValue=""
              value={currentFilterValues.taskType}
              onChange={(e) => {
                setFilters([
                  {
                    field: 'propertyType',
            ...
            >
              ...
              {['Work', 'Errands', 'Meetings', 'Maintenance', 'Administrative']
              .map((type) => (
